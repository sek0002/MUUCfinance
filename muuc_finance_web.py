...
def format_axis_tick_label(bucket: str, window_key: str) -> str:
    try:
        if window_key in {"day", "week", "month"}:
            if window_key == "week":
                bucket = str(bucket).split("/")[0]
            return pd.to_datetime(bucket, errors="raise").strftime("%b %Y")
        if window_key == "year":
            return pd.to_datetime(f"{bucket}-01-01", errors="raise").strftime("%Y")
    except Exception:
        return str(bucket)
    return str(bucket)


def visible_axis_tick_labels(labels: list[str], window_key: str) -> list[str]:
    visible_labels: list[str] = []
    last_display_label = None
    for bucket in labels:
        display_label = format_axis_tick_label(bucket, window_key)
        if display_label == last_display_label:
...
def build_line_chart_svg(series_map: dict[str, pd.Series], title: str, window_key: str) -> str:
    ...
    visible_tick_labels = visible_axis_tick_labels(labels, window_key)
    ...
    for index, display_label in enumerate(visible_tick_labels):
...
def build_category_summary_line_chart_svg(income_totals: pd.Series, expense_totals: pd.Series, title: str) -> str:
    ...
    if not labels:
        return '<div class="chart-empty">No data in the selected range.</div>'
    visible_tick_labels = visible_axis_tick_labels(labels, "year")
...
def rules_context(request: Request, message: Optional[str], view: str = "All") -> dict[str, Any]:
    ...
    category_summary_rows = category_subgroup_rows(bundle.income, bundle.expenses)
